...
window.Radar = (() => {
  ...
  const chains = {
    valid: "Доверена",
    valid_but_expired: "Истёк",
    self_signed: "Самоподписанный",
    untrusted_root: "Недоверенный CA",
    incomplete_or_untrusted: "Неполная / недоверенная",
  ...
  async function api(path, options = {}) {
    ...
    try {
      ...
      throw new Error("Нет связи с сервером. Проверьте, что приложение запущено.");
    }
  ...
  const risk = row => el("span", row.risk_score === null ? "—" : `${row.risk_score} / ${row.risk_level}`,
    "badge risk-" + (row.risk_level || "none"));
  ...
  function action(selector, callback) {
    document.querySelector(selector)?.addEventListener("click", async event => {
      ...
      try {
        ...
        if (!button.dataset.busy) button.disabled = false;
      }
    });
  }
